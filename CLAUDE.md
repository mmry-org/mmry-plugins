...
- `collection` - Collection name/ID for categorization
- Custom properties allowed

### Collections

Collections are used to categorize and organize items:

- **defaultCollection**: Define a default collection in plugin manifest
- **Runtime**: Set collection via `mmry.add({ collection: "name", ... })`
- **Organization**: Items can be filtered and grouped by collection
...
- Environment variables prefixed with MMRY\_ are reserved for the system
